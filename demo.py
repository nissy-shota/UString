...
def extract_features(detector, feat_extractor, video_file, n_frames=100, n_boxes=19):
    ...
    features = np.zeros(
        (n_frames, n_boxes + 1, feat_extractor.dim_feat), dtype=np.float32
    )
    ...
    for idx in range(n_frames):
        ...
        features[idx, 0, :] = (
            np.squeeze(feature_frame.cpu().numpy())
            if feature_frame.is_cuda
            else np.squeeze(feature_frame.detach().numpy())
        )
        features[idx, 1:, :] = (
            np.squeeze(feature_roi.cpu().numpy())
            if feature_roi.is_cuda
            else np.squeeze(feature_roi.detach().numpy())
        )
        frame_prev = frame
    return detections, features


def init_accident_model(
    model_file,
    dim_feature=4096,
    hidden_dim=256,
    latent_dim=256,
...
def load_input_data(feature_file, device=torch.device("cuda")):
    ...
    def generate_st_graph(detections):
        ...
        for i in range(num_frames):
            ...
            edge_weights[i] = compute_graph_edge_weights(
                detections[i, :, :4], edge
            )  # 171,
    ...
    def compute_graph_edge_weights(boxes, edges):
        ...
        for i, edge in enumerate(edges):
            c1 = [
                0.5 * (boxes[edge[0], 0] + boxes[edge[0], 2]),
                0.5 * (boxes[edge[0], 1] + boxes[edge[0], 3]),
            ]
            c2 = [
                0.5 * (boxes[edge[1], 0] + boxes[edge[1], 2]),
    ...
    features = torch.Tensor(np.expand_dims(features, axis=0)).to(
        device
    )  #  50 x 20 x 4096
...
def load_checkpoint(
    model, optimizer=None, filename="checkpoint.pth.tar", isTraining=True
):
    ...
    if os.path.isfile(filename):
        ...
        pretrained_dict = {
            ...
            if not any(
                filtered in k for filtered in ["self_aggregation", "predictor_aux"]
            )
        }
...
def parse_results(all_outputs, batch_size=1, n_frames=50):
    ...
    for t in range(n_frames):
        ...
        aleatoric = (
            aleatoric.cpu().numpy() if aleatoric.is_cuda else aleatoric.detach().numpy()
        )
        epistemic = all_outputs[t]["epistemic"]  # B x 2 x 2
        epistemic = (
            epistemic.cpu().numpy() if epistemic.is_cuda else epistemic.detach().numpy()
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="visualize",
        choices=["extract_feature", "inference", "visualize"],
    )
    parser.add_argument("--gpu_id", help="GPU ID", type=int, default=0)
    parser.add_argument(
        "--n_frames", type=int, help="The number of input video frames.", default=50
    )
    parser.add_argument("--seed", type=int, help="The random seed.", default=123)
    parser.add_argument(
        "--fps", type=float, help="The fps of input video.", default=10.0
    )
    parser.add_argument(
        "--fps_display", type=float, help="The fps of output video.", default=2.0
    )
    # feature extraction
    ...
    parser.add_argument(
        "--mmdetection",
        type=str,
        help="the path to the mmdetection.",
        default="lib/mmdetection",
    )
    # inference
    parser.add_argument(
        "--feature_file",
        type=str,
        help="the path to the feature file.",
        default="demo/000821_feature.npz",
    )
    parser.add_argument(
        "--ckpt_file",
        type=str,
        help="the path to the model file.",
        default="demo/final_model_ccd.pth",
    )
    # visualize
    parser.add_argument(
        "--result_file",
        ...
        default="demo/000821_result.npz",
    )
    parser.add_argument(
        "--vis_file",
        type=str,
        help="the path to the visualization file.",
        default="demo/000821_vis.avi",
    )
    p = parser.parse_args()
    ...
    device = (
        torch.device("cuda:" + str(p.gpu_id))
    ...
    if p.task == "extract_feature":
        ...
        cfg_file = osp.join(
            p.mmdetection, "configs/cascade_rcnn_x101_64x4d_fpn_1x_kitti2d.py"
        )
        model_file = osp.join(
            p.mmdetection, "work_dirs/cascade_rcnn_x101_64x4d_fpn_1x_kitti2d/latest.pth"
        )
        ...
        detections, features = extract_features(
            detector, feat_extractor, p.video_file, n_frames=p.n_frames
        )
        ...
        (
            ...
            detections,
            vid,
        ) = load_input_data(p.feature_file, device=device)
        # prepare model
        model = init_accident_model(
            p.ckpt_file, dim_feature=features.shape[-1], n_frames=p.n_frames, fps=p.fps
        )
        with torch.no_grad():
            # run inference
            _, all_outputs, _ = model(
                features,
                labels,
                toa,
                graph_edges,
        ...
        result_file = osp.join(
            osp.dirname(p.feature_file),
            p.feature_file.split("/")[-1].split("_")[0] + "_result.npz",
        )
        np.savez_compressed(
            result_file,
            score=pred_score[0],
        ...
        pred_score, aleatoric, epistemic, detections = (
            all_results["score"],
            all_results["aleatoric"],
            all_results["epistemic"],
            all_results["det"],
        )
        xvals, pred_score, std_alea, std_epis = preprocess_results(
        ...
        plt.xticks(
            range(0, len(xvals) + 1, int(p.n_frames / p.fps_display)), fontsize=fontsize
        )
        plt.yticks(fontsize=fontsize)

        from matplotlib.animation import FFMpegWriter

        curve_writer = FFMpegWriter(
            fps=p.fps_display,
            metadata=dict(
        ...
        for t, frame in enumerate(video_data):
            ...
            frame[frame.shape[0] - height : frame.shape[0]] = cv2.addWeighted(
                frame[frame.shape[0] - height : frame.shape[0]], 0.3, img, 0.7, 0
            )
